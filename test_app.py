...
st.sidebar.markdown("---")
st.sidebar.header("🎨 圖表顯示控制 (視覺優化)")
show_patterns = st.sidebar.checkbox("顯示 AI 幾何型態線條", value=False) # 預設關閉，畫面更乾淨！
        ...
        with st.spinner("正在讀取歷史數據並分析型態..."):
            try:
                ...
                if df.empty:
                    ...
                    with col_p1:
                        st.subheader("🔍 AI 型態偵測結果")
                        pattern_options = ["不顯示型態 (清爽模式)", "顯示全域所有型態"] + [f"{p['date']} {p['name']}" for p in detected_patterns]
                        selected_option = st.selectbox("選擇要在圖表繪製的型態：", pattern_options)
                    
                    with col_p2:
                        if detected_patterns:
                            st.info(f"💡 區間內共掃描出 **{len(detected_patterns)}** 個型態。可在左側選單點選特定型態進行高亮標註。")
                        else:
                            st.warning("該區間內未辨識出明顯型態。")
                    ...
                    # 幾何型態標註邏輯 (依使用者選擇繪製)
                    patterns_to_draw = []
                    if selected_option == "顯示全域所有型態" or show_patterns:
